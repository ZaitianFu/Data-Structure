...
class KDTree
{
  public:
    ...
    bool smallerDimVal(const Point<Dim>& first, const Point<Dim>& second,
                       int curDim) const;
double finddis(const Point<Dim>& p1,const Point<Dim>& p2)const;
    /**
	...
	void KDTreehelper(vector< Point<Dim> > & newPoints, int L, int R, int dimension);
	Point<Dim> Quickselect(vector< Point<Dim> > & newPoints, int L, int R, int mid, int dimension);
    /**
...
Point<Dim> findNearestNeighborhelper(int dimension, const Point<Dim> &query, int L, int R, const Point<Dim> &currentBest) const;
